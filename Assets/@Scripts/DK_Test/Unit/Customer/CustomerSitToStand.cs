...
public class MusteriSitToStand : CustomerBaseState
{
    ...
    public override void StartState(Action_Test action)
    {
        ...
        customer.agent.SetDestination(Door.instance.doorTransform.position);
    }
    public override void UpdateState(Action_Test action)
    {
        if( Vector3.Distance(customer.agent.transform.position,Door.instance.doorTransform.position) > 0.5f)
        {
            return;
        }
